...
const languageOptions = [
    { value: 'html', label: 'HTML/JS Canvas' },
    { value: 'phaser', label: 'Phaser.js' },
    { value: 'kaboom', label: 'Kaboom.js' }
];

export const GameBuilder: FC = () => {
    ...
    return (
        <div className="flex flex-col h-full animate-fade-in">
            <h2 className="text-2xl font-semibold mb-2 text-indigo-300">سازنده بازی</h2>
            <p className="mb-4 text-gray-400">یک بازی ساده تحت وب را توصیف کنید تا "اوستا" آن را برایتان بسازد.</p>
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                <textarea
                    value={prompt}
                    onChange={(e) => setPrompt(e.target.value)}
                    placeholder="مثال: یک بازی ساده مار که با کلیدهای جهت‌نما کنترل می‌شود."
                ...
                <div className="flex flex-wrap items-center gap-4">
                    <button type="submit" className="px-6 py-2 bg-indigo-600 rounded-lg hover:bg-indigo-500 disabled:bg-gray-600 disabled:cursor-not-allowed transition font-semibold" disabled={isLoading || isEditing}>
                     ...
                     <div className="flex items-center gap-2">
                        <label htmlFor="language-select" className="text-sm font-medium text-gray-300">موتور بازی:</label>
            ...
            {files.length > 0 && !isLoading && (
                <form onSubmit={handleEditSubmit} className="mt-4 p-4 bg-gray-800/50 rounded-lg border border-gray-700/50 flex flex-col gap-3 animate-fade-in">
                    ...
                    <textarea
                        ...
                        placeholder="تغییرات مورد نظر خود را توصیف کنید. مثلا: سرعت بازیکن را بیشتر کن."
                        ...
                        {isLoading || isEditing ? (
                            <div className="flex items-center justify-center h-full">
                                <div className="w-12 h-12 border-4 border-indigo-400 border-t-transparent rounded-full animate-spin"></div>
                                 <span className="ml-4 text-gray-400">{isEditing ? 'در حال اعمال تغییرات...' : 'در حال ساخت بازی...'}</span>
                            </div>
                        ) : previewHtml ? (
                            <iframe
                                srcDoc={previewHtml}
                            ...
                            <div className="flex items-center justify-center h-full text-gray-500 p-4 text-center">
                                پیش‌نمایش بازی شما اینجا نمایش داده می‌شود.
                            </div>
                        )}
                    </div>
                </div>
            ...
            <div className="flex items-center gap-2 p-2 mt-4 text-sm text-gray-400 bg-gray-800/50 rounded-lg border border-gray-700/50">
                ...
                <span>محدودیت استفاده (نمایشی): ۱۰ بازی در روز.</span>
            </div>
        </div>
    );
};