...
export const MusicFinder: FC = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const fullPrompt = `Please identify the song from this URL: ${prompt}. Analyze the content at the URL to find the song title and artist.`;
  ...
  return (
    <div className="flex flex-col h-full animate-fade-in">
      ...
      <p className="mb-4 text-gray-400">لینک ویدیو از یوتیوب، اینستاگرام و... را وارد کنید تا اوستا سعی کند آهنگ آن را پیدا کند.</p>
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        <input
          type="url"
          value={prompt}
          onChange={(e) => setPrompt(e.target.value)}
          placeholder="لینک ویدیو را اینجا جای‌گذاری کنید..."
        ...
        {!isLoading && result.text && !error && (
          <div>
            ...
            {result.sources.length > 0 && (
              <div className="mt-4">
                <h4 className="font-semibold text-indigo-200">منابع:</h4>
      ...
      <div className="flex items-center gap-2 p-2 mt-4 text-sm text-gray-400 bg-gray-800/50 rounded-lg border border-gray-700/50">
        ...
        <span>موفقیت این ابزار به در دسترس بودن اطلاعات عمومی در مورد لینک بستگی دارد.</span>
      </div>
    </div>
  );
};