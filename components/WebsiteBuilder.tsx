...
export const WebsiteBuilder: FC = () => {
    ...
    useEffect(() => {
        if (files.length > 0) {
            ...
            setPreviewHtml(indexFile ? indexFile.content : '<html><body>فایل index.html برای پیش‌نمایش یافت نشد.</body></html>');
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            const responseFiles = await geminiService.generateCodeProject(prompt, 'website', language, mode);
    ...
    const handleEditSubmit = async (e: FormEvent) => {
        ...
        try {
            const responseFiles = await geminiService.editCodeProject(files, editInstruction, 'website', language, mode);
            setFiles(responseFiles);
    ...
    return (
        <div className="flex flex-col h-full animate-fade-in">
            <h2 className="text-2xl font-semibold mb-2 text-indigo-300">سازنده وب‌سایت</h2>
            <p className="mb-4 text-gray-400">وب‌سایتی که می‌خواهید را توصیف کنید تا "اوستا" کد آن را برایتان بسازد.</p>
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                <textarea
                    value={prompt}
                    onChange={(e) => setPrompt(e.target.value)}
                    placeholder="مثال: یک صفحه شخصی برای یک عکاس با یک گالری عکس زیبا و یک فرم تماس."
                ...
                <div className="flex flex-wrap items-center gap-4">
                    ...
                    <div className="flex items-center gap-2">
                        <label htmlFor="language-select" className="text-sm font-medium text-gray-300">فریمورک:</label>
            ...
            {files.length > 0 && !isLoading && (
                <form onSubmit={handleEditSubmit} className="mt-4 p-4 bg-gray-800/50 rounded-lg border border-gray-700/50 flex flex-col gap-3 animate-fade-in">
                    ...
                    <textarea
                        ...
                        placeholder="تغییرات مورد نظر خود را توصیف کنید. مثلا: رنگ پس‌زمینه را به آبی تیره تغییر بده."
            ...
            <div className="mt-6 flex-1 grid grid-cols-1 lg:grid-cols-2 gap-6 overflow-hidden">
                ...
                <div className="flex flex-col overflow-hidden">
                    <div className="flex justify-between items-center mb-2">
                        <h3 className="text-lg font-semibold text-gray-200 flex-shrink-0">پیش‌نمایش</h3>
                    </div>
                    <div className="flex-1 border border-gray-700 rounded-lg bg-gray-950/50 overflow-hidden">
                        {isLoading || isEditing ? (
                            <div className="flex items-center justify-center h-full">
                                <div className="w-12 h-12 border-4 border-indigo-400 border-t-transparent rounded-full animate-spin"></div>
                                <span className="ml-4 text-gray-400">{isEditing ? 'در حال اعمال تغییرات...' : 'در حال ساخت پروژه...'}</span>
                            </div>
                        ) : previewHtml ? (
                            <iframe
                                srcDoc={previewHtml}
                            ...
                            <div className="flex items-center justify-center h-full text-gray-500 p-4 text-center">
                                پیش‌نمایش وب‌سایت شما اینجا نمایش داده می‌شود.
                            </div>
                        )}
                    </div>
                </div>
                           ...
                           <div className="flex items-center justify-center h-full text-gray-500">...</div>
                            ...
                            <>
                                ...
                                <div className="flex-1 relative overflow-auto p-1">
                                     <button onClick={handleCopy} className="absolute top-2 left-2 px-3 py-1 text-xs bg-gray-700 hover:bg-gray-600 rounded-lg font-semibold z-10">
            ...
            <div className="flex items-center gap-2 p-2 mt-4 text-sm text-gray-400 bg-gray-800/50 rounded-lg border border-gray-700/50">
                ...
                <span>محدودیت استفاده (نمایشی): ۱۰ پروژه در روز.</span>
            </div>
        </div>
    );
};